...
def add_daily_features(daily_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Lag/rolling features. These must use only previous AQI values.
    ...
    df["aqi_rolling_mean_3d"] = grouped["aqi_mean"].shift(1).rolling(3).mean().reset_index(level=0, drop=True)
    df["aqi_rolling_mean_7d"] = grouped["aqi_mean"].shift(1).rolling(7).mean().reset_index(level=0, drop=True)
...
def make_training_frame(hourly_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Drop rows without enough AQI history for lag/rolling features.
    # Target is same-day daily AQI. Future rows will use forecast pollutants/weather + recursive AQI lags.
